          ...
          return (
            (key === "first" || key === "last") && (
              <div key={key} className={`w-full`}>
                ...
                <input
                  type="text"
                  className={`form-input[type='text'] font-sans w-full bg-white p-2 text-black border-[1px] border-black`}
                  name={String(key)}
                  id={`input_${formId}_${id}_${key}`}
                  placeholder={
                    isRequired ? `${placeholder || inputLabel}*` : (placeholder || inputLabel)
                  }
