...
const Form = ({ className, title, content, form }: FormProps) => {
  ...
  return (
    <div className={`${className} relative my-4`}>
      <div className={`max-w-[1300px] mx-auto flex flex-col p-4 gap-6 text-center`}>
        {title &&
          <h2 className={`font-heading text-2xl md:text-4xl font-bold text-black`}>
        ...
        {content &&
          <Content
            className={`mx-auto my-4 font-sans text-black`}
            content={content}
          />
        }
        
        <GravityForm form={formData} formId={form as any ?? 1} />
      </div>
    </div>
  );
};
